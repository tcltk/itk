...
#
# RCS: @(#) $Id: menu_1.tcl,v 1.1.2.1 2007/11/29 21:07:59 wiede Exp $
#--------------------------------------------------------------------------
...
proc callOranges {args} {
puts stderr "callOranges called!$args!"
}

proc callBananas {args} {
puts stderr "callBananas called!$args!"
}

::ntk::glmwfw::Glmwfw setWindowTitle $::ntk::_win "NtkWidget Menu Demo 1"

set rotate 0
if {[llength $argv] > 0} {
    set rotate [lindex $argv 0]
}
set wPath [ntk menu .w -width 100 -height 100 -xoffset 50 -yoffset 50 \
-bd 2 -bg [list 175 175 175 255]]
$wPath configure -rotate $rotate
$wPath addentry oranges callOranges
$wPath addentry bananas callBananas
$wPath addseparator 
$wPath addmenu submenu1
